...
class GradientBackground {
  ...
  constructor(sceneManager: any) {
    ...
    this.uniforms = {
      ...
      // Custom colors: Light blue and black
      uColor1: { value: new THREE.Vector3(0.353, 0.616, 0.980) }, // Light blue #5A9DF9
      uColor2: { value: new THREE.Vector3(0.035, 0.043, 0.090) }, // Near black #090B17
      uColor3: { value: new THREE.Vector3(0.463, 0.749, 1.000) }, // Sky blue #76BFFF
      uColor4: { value: new THREE.Vector3(0.000, 0.000, 0.000) }, // Black
      uColor5: { value: new THREE.Vector3(0.278, 0.549, 0.941) }, // Blue #478CF0
      uColor6: { value: new THREE.Vector3(0.122, 0.180, 0.349) }, // Deep blue #1F2E59
      uSpeed: { value: 1.2 }, uIntensity: { value: 1.8 },
      uTouchTexture: { value: null }, uGrainIntensity: { value: 0.08 },
      uDarkNavy: { value: new THREE.Vector3(0.000, 0.000, 0.000) }, // Black
      uGradientSize: { value: 0.45 }, uGradientCount: { value: 12.0 },
      uColor1Weight: { value: 0.6 }, uColor2Weight: { value: 1.6 }
    };
  }
  init() {
  ...
  setTheme(isDark: boolean) {
    if (isDark) {
      // Dark mode - light blue and black
      this.uniforms.uColor1.value.set(0.353, 0.616, 0.980); // Light blue
      this.uniforms.uColor2.value.set(0.035, 0.043, 0.090); // Near black
      this.uniforms.uColor3.value.set(0.463, 0.749, 1.000); // Sky blue
      this.uniforms.uDarkNavy.value.set(0.000, 0.000, 0.000); // Black
      this.sceneManager.scene.background = new THREE.Color(0x000000);
    } else {
      // Light mode - softer blues with light background
      this.uniforms.uColor1.value.set(0.62, 0.78, 1.0);
      this.uniforms.uColor2.value.set(0.92, 0.95, 1.0);
      this.uniforms.uColor3.value.set(0.50, 0.70, 0.98);
      this.uniforms.uDarkNavy.value.set(0.95, 0.97, 1.0);
      this.sceneManager.scene.background = new THREE.Color(0xf5f9ff);
    }
  }
...
export default function LiquidGradient({ 
  ...
  subtitle = "Bridging Engineering, Research, and Design",
  ...
  return (
    <div className={`relative w-full h-full ${className}`}>
      ...
      {showCta && (
        <button 
          className="absolute bottom-20 left-1/2 -translate-x-1/2 z-10 px-8 py-3 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full text-white font-medium hover:bg-white/20 transition-all"
          onClick={onCtaClick}
        >
          {ctaText}
        </button>
      ...
      {showPauseButton && (
        <button
          ...
          className="absolute bottom-4 right-4 z-10 p-3 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full text-white hover:bg-white/20 transition-all"
      ...
      {/* Footer */}
      {showFooter && (
        <footer className="absolute bottom-4 left-4 z-10 text-white/50 text-sm">
          <a href="https://haikkashiyani.kesug.com/?i=1" target="_blank" rel="noopener noreferrer" className="hover:text-white/80 transition-colors">
            Made By Haik
          </a>
        </footer>
      )}
